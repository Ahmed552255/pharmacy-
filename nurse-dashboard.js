...
async function loadDrugsFromJSON() {
    try {
        // ملاحظة: تأكد من وجود الملف في نفس المجلد
        ...
        // عرض عينة للتأكد
        if (drugsFromJSON.length > 0) {
            console.log('عينة:', drugsFromJSON.slice(0, 3));
        }
    } catch (err) {
...
function searchDrugs(term, selectedForm = null) {
    ...
    return uniqueMatches.slice(0, 15); // زيادة عدد النتائج
}

// ---------- عرض الاقتراحات في واجهة المستخدم ----------
function displayDrugSuggestions(term) {
    // إمكانية فلترة حسب الشكل المختار حالياً
...
// ---------- لوحة الجرعات الذكية (محسنة مع اقتراحات توقيت متقدمة) ----------
...
UI.drugFormSelect.addEventListener('change', () => {
    updateUnitLabel();
    // إذا كان هناك نص مكتوب، يمكن تحديث الاقتراحات مع الشكل الجديد
...
function generateDoseSuggestions() {
    ...
    // اقتراحات تلقائية بناءً على الكمية
...
UI.applyDoseBtn.addEventListener('click', () => {
    ...
    if (activeRow) {
        ...
        // إذا كان الصف يحتوي على توقيت افتراضي (من الاقتراح نفسه)
...
(async function init() {
    ...
    // إذا لم يتم تحميل أي دواء من JSON، نعرض رسالة في وحدة التحكم
    if (drugsFromJSON.length === 0) {
        console.warn('⚠️ لم يتم تحميل أي دواء من ملف JSON. تأكد من وجود الملف وصحة مساره.');
    }
